...
This doc is not mine, it was tken from this repo: https://github.com/jekil/hardentheworld
...
Don't destroy FileVault Keys (Because you loose your session every time it goes in standy mode)
^^^^^^^^^^^^^^^^^^^^^^
...
It is (not) suggested to configure your system to destroy File Vault keys when enterging
...
(Do not) power off memory during standy
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

By default during stand-by memeory are kept powered on, this is prone to
forensics acquisition of your memory. 

Right but it takes a long time to wake up, and you can potentially 
loose your session. Do it only if you know what you're doing.
...
It is (not) suggested to power off memory at stand-by with the following command::
...
Paranoid Users
^^^^^^^^^^^^^^
For paranoid users which want privacy, check this : https://github.com/drduh/macOS-Security-and-Privacy-Guide
